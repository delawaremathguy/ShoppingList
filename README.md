...
* ~~I discovered a regression~~: suddenly an edit of a ShoppingItem was not being respected.  turns out I had set `var itemData: ShoppingItemRowData` in ShoppingItemRowView to be `@State`.  that was a terrible idea.  removing `@State` fixed the problem.
