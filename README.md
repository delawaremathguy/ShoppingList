...
My Last Update of note was **August 1, 2020**, when these were some of the recent changes I made.

* I have pretty much completed a major rewrite of the code for lists of shopping items and locations to use their own, proper "view model," at least in the sense that I understand the term.  In particular, I am **no longer using @FetchRequest** -- anywhere!  In short, the view models take the place of @FetchRequest, but the  inner workings of the model are now clear to me, and I can tell where things break.  This was the major problem I was having with @FetchRequest: I was never sure how, when, and in what sequence it worked, or why things broke when they did.

* To make the view models work correctly, I used a very small portion of Combine code so that any changes to a ShoppingItem or a Location will be seen by the view model, which  will then emit its own willChangeObject.send(). This turned out to be relatively easy; and, along the way, I learned much more detail about how SwiftUI works.  Well, I *think* I learned more about it, but my working theory of the case is now much better and consistent in understanding all the previous problems I was having when using @FetchRequest.

* I really do now believe that while @FetchRequest is a convenience for many simple cases, but I think it breaks the MVVM architecture.  Instead of asking a view model to make some changes to a Core Data object, you just make the change yourself, directly on the object, and then expect that @FetchRequest sees the change and does the right thing.  But there is no centralized editing of objects: if code starts breaking, you have to look through a lot of source code to find out where some Core Data object was edited or not edited differently than it was somewhere else.  And the problem that I kept running into with @FetchRequest concerned *deletions* of Core Data objects being managed by a @FetchRequest, and I was never convinced it "did the right thing."

* With this rewrite, the deletion bug issue seems to have disappeared, *I think* (XCode 11.6/iOS 13.6), and the reason is that I am not letting SwiftUI use @FetchRequest to manage a list of items for me.  The crash I kept getting was always the same: a Core Data object was deleted, but the @FetchRequest seemed to be still be using a phantom reference to it where .isDeleted is false, but .isFault is true.  Everytime you tried to access the optional fields of the object, we go BOOM because it's not a real object. There's an issue of timing and coordination here that just was not working right (*or I was not understanding correctly*).
...
I'm have, briefly, tesxted out this code with XCode 12beta 3, and here are some observations so far:
