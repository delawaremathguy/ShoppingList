...
## Last Update of Note: September 20, 2020


My apologies for prematurely posting the project yesterday for XCode 12 (despite my comments, the code was still not compiling in XCode 11).  The status of XCode 12 right now is [highly questionable and downright frustrating for several devs](https://developer.apple.com/forums/thread/660490).  Everyone seems to be scrambling, especially with the mismanaged release of XCode 12 through the App Store.

Thus I have reverted to posting the project with XCode 11.7. *If you do use XCode 12 or XCode 12.2 beta, and you should be aware of some items*:

* (*Identifiable conformance of Core Data classes*) In XCode 12, conformance of ShoppingItem and Location to Identifiable is automatically added in the code auto-generated by XCode and you will receive *duplicate conformance* errors with this code.  Simply remove the file IdentifiableConformance11.7.swift from the project.
...
* The code in SLSectionHeaderView.swift has several lines of code commented out so this will compile under XCode 11. The code will compile under XCode 12 as is, but you may want to reinstate those lines so that the code runs with a similar section header appearance as in iOS 13.
