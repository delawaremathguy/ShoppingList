...
My Last Update of note was **September 4, 2020**. Other than for future bug fixes (i.e., *removal of coding anomalies*) and perhaps some updating of comments within the code for things that have evolved during development, **this was the last, public update of the project**.

* As of September 18, any update I post will be with XCode 12 (the current GM release version 12A7209), and you should be aware of some items if you are still in XCode 11*:

* (*Identifiable conformance of Core Data classes*) In XCode 12, conformance of ShoppingItem and Location to Identifiable is automatically added in the code auto-generated by XCode.  If you are using XCode 11, you **must un-comment the extension code** at the top of ShoppingItem+Extensions.swift and Location+Extensions.swift (one line each!) to tell XCode 11 that these clases are extended to conform to Identifiable.
...
* (*Use of if #availlable*) The code for SLSectionHeaderView uses this conditional, so that if running in iOS 14, we change the `textCase` of a Section header title and center it in the header (consistent with iOS 13).  
